...
export function PermitsBrowser({ counties, initialStats }: PermitsBrowserProps) {
  ...
  async function handleSync(county: string) {
    ...
    try {
      await fetch('/leads/api/permits/sync', {
        method: 'POST',
      ...
      // Refresh stats
      const freshStats = await fetch('/leads/api/permits/stats').then(r => r.json()) as Record<string, CountyStat>
      setStats(freshStats)
  ...
  async function handleBulkSync() {
    ...
    setSyncMsg(null)
    try {
      const res = await fetch('/leads/api/permits/bulk-sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleRematch() {
    ...
    setRematchMsg(null)
    try {
      const res = await fetch('/leads/api/permits/rematch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ county: selectedCounty }),
      })
                      ...
                      {permits.map(permit => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5">
                            {permit.companyId ? (
                              <Link
                                href={`/leads/companies/${permit.companyId}`}
