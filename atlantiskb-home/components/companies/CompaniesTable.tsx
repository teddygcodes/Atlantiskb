...
export function CompaniesTable({ companies, pagination }: CompaniesTableProps) {
  ...
  async function handleEnrichSelected() {
    ...
    try {
      const res = await fetch('/leads/api/enrich/batch', {
        method: 'POST',
  ...
  async function handleDeleteSelected() {
    ...
    try {
      const res = await fetch('/leads/api/companies/batch-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ companyIds: [...selectedIds] }),
      })
  ...
  async function handleEnrichAll() {
    ...
    try {
      ...
      const queueRes = await fetch('/leads/api/enrich/batch')
      if (!queueRes.ok) {
  ...
  async function handleRescore() {
    ...
    try {
      const res = await fetch('/leads/api/rescore', { method: 'POST' })
      const data = await res.json()
      if (!res.ok) {
  ...
  async function handleMerge(primaryId: string, secondaryId: string) {
    ...
    setMergeStatus(null)
    try {
      const res = await fetch('/leads/api/companies/merge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
