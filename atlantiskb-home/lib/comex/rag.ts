...
export async function buildRAGContext(question: string, metals: MetalKey[]): Promise<RAGContext> {
  ...
  const similarRows = await db.$queryRaw<SimilarityRow[]>(Prisma.sql`
    SELECT
      "id",
      "snippet",
      "url",
  ...
  for (const row of similarRows) {
    ...
    const relatedEvents = await db.$queryRaw<PriceEventRow[]>(Prisma.sql`
      SELECT
        "id",
        "metal",
        "date",
  ...
  for (const metal of METAL_KEYS) {
    ...
    const recentLargeEvents = await db.$queryRaw<PriceEventRow[]>(Prisma.sql`
      SELECT
        "id",
        "metal",
        "date",
